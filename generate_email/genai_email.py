...
def get_response(user, target, selected_service):
    ...
    framework_instruction = FRAMEWORK_GUIDELINES.get(target.framework, "")

    system_prompt = """
You are a B2B cold email specialist.

You write short, direct emails that sound like a business professional wrote them — not a marketer.

RULES YOU NEVER BREAK:
- You NEVER fabricate facts, statistics, or case studies.
- You NEVER use placeholder text other than the variables supplied.
- You focus on business outcomes, not product features.
- You use industry-specific language appropriate to the prospect.
- You mention the product or company name once or twice at most.

BANNED PHRASES — never use these under any circumstance:
"In today's fast-paced world", "I hope you're doing well", "Imagine",
"I see an opportunity", "Enhance efficiency", "Streamline operations",
"Optimize workflows", "Robust solution", "Cutting-edge technology",
"Industry-leading", "Leverage", "Digital transformation",
"Technology-driven operations", "Growth objectives"
"""

    user_prompt = f"""
Write a B2B cold email sequence using the following inputs.

---

SENDER COMPANY: {user.company_name}
SENDER NAME: {user.get_full_name()}

RECIPIENT: {target.receiver_first_name} {target.receiver_last_name}
COMPANY WEBSITE: {target.company_url}

CAMPAIGN GOAL: {target.campaign_goal}

SERVICE NAME: {selected_service.service_name}
SERVICE USP: {selected_service.product_usp}

COMPANY SIGNALS:
- Primary: {scraped_data['primary_signal']}
- Supporting: {scraped_data['supporting_signal']}
- Additional: {scraped_data['growth_signals']}

---

SELECTED FRAMEWORK: {target.framework}

{framework_instruction}

The email paragraphs MUST follow the persuasion logic and flow of the selected framework above.
Do NOT name or label the framework anywhere in the email.
The framework shapes how paragraphs transition and persuade — it must be invisible to the reader.

---

MAIN EMAIL STRUCTURE (follow exactly):

Greeting: Hi {target.receiver_first_name},

Paragraph 1 — Industry Problem:
  Identify a specific operational challenge relevant to the prospect's industry or role.
  Do NOT mention the product here.
  1–2 sentences only.

Paragraph 2 — Solution:
  Introduce {selected_service.service_name} and briefly explain how it addresses the problem above.
  Derive the problem it solves and outcome it drives from the USP — do not repeat USP text verbatim.
  1–2 sentences only.

Paragraph 3 — Outcome Bullets:
  List 3–5 concrete business outcomes the prospect would gain.
  Use bullet points.
  Each bullet = one outcome, 6–10 words max.
  Draw from outcomes like: reduce operational workload, scale without increasing headcount,
  accelerate onboarding, reduce turnaround time, improve response times,
  increase client engagement, deliver reports instantly, reduce manual effort,
  improve customer experience.
  Adapt outcomes to the specific service and industry — do not copy these verbatim.

Paragraph 4 — CTA:
  End with a single low-friction question.
  Example: "Would it be worth a brief conversation to explore?"
  Do NOT say: "schedule a call", "quick call", "let me know if you're interested".

Word count: 80–120 words excluding greeting.

---

SUBJECT LINES: Generate 3.
Rules: 3–6 words, contextual, natural, no clickbait.

---

FOLLOW-UPS: Write 4 follow-up emails.

Rules for each follow-up:
- MUST begin with: Hi {target.receiver_first_name},
- 50–90 words excluding greeting
- Each introduces a NEW angle — no repetition of the main email
- No "just following up" or "checking in"
- End each with a single CTA question
- Do NOT use any banned phrases

Follow-up angles:
  F1 → Expand on a specific outcome or use case relevant to their industry
  F2 → Share a relevant operational observation (no fabricated stats or names)
  F3 → Offer a small, concrete idea or framing they may not have considered
  F4 → Polite close — start with an action-oriented sentence, offer to step back if not relevant
...
OUTPUT RULES:
- Clean HTML only (use <p> for paragraphs, <ul><li> for bullets)
- No markdown
- No signature block
- No placeholders other than those already resolved above
...
"""
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Format:\n"
                    "{\n"
                    "  'main_email': { 'subject_options': [], 'body': '' },\n"
                    "  'follow_ups': [ { 'body': '' } ]\n"
                    "}"
                )
            },
    ...
    data["framework"] = (getattr(target, "framework", "") or "").strip()
