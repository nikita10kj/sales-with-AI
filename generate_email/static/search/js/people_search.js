document.addEventListener("DOMContentLoaded", function () {
    ...
    function setupTagInput(config) {
        ...
        tagInputInstances.push({
            finalizePendingInput: finalizePendingInput
        });
    }
    ...
    setupTagInput({
        inputId: "nameInput",
        addBtnId: "addNameTag",
        tagsContainerId: "nameTags",
        hiddenInputId: "nameHidden"
    });

    setupTagInput({
        inputId: "locationInput",
        addBtnId: "addLocationTag",
        tagsContainerId: "locationTags",
        hiddenInputId: "locationHidden"
    });

    setupTagInput({
        inputId: "companyInput",
        addBtnId: "addCompanyTag",
        tagsContainerId: "companyTags",
        hiddenInputId: "companyHidden"
    });

    setupTagInput({
        inputId: "specialitesInput",
        addBtnId: "addSpecialitesTag",
        tagsContainerId: "specialitesTags",
        hiddenInputId: "specialitesHidden"
    });

    setupTagInput({
        inputId: "industryInput",
        addBtnId: "addIndustryTag",
        tagsContainerId: "industryTags",
        hiddenInputId: "industryHidden"
    });

    setupTagInput({
        inputId: "jobTitleInput",
        addBtnId: "addJobTitleTag",
        tagsContainerId: "jobTitleTags",
        hiddenInputId: "jobTitleHidden"
    });

    setupTagInput({
        inputId: "skillsInput",
        addBtnId: "addSkillsTag",
        tagsContainerId: "skillsTags",
        hiddenInputId: "skillsHidden"
    });

    setupTagInput({
        inputId: "institutionInput",
        addBtnId: "addInstitutionTag",
        tagsContainerId: "institutionTags",
        hiddenInputId: "institutionHidden"
    });

    setupTagInput({
        inputId: "degreeInput",
        addBtnId: "addDegreeTag",
        tagsContainerId: "degreeTags",
        hiddenInputId: "degreeHidden"
    });
    ...
    if (peopleSearchForm) {
        peopleSearchForm.addEventListener("submit", function () {
            ...
            console.log("name =", document.getElementById("nameHidden")?.value);
            console.log("company =", document.getElementById("companyHidden")?.value);
            console.log("location =", document.getElementById("locationHidden")?.value);
            console.log("skills =", document.getElementById("skillsHidden")?.value);
        });
    }
    ...
    if (closeSaveListModal) {
        closeSaveListModal.addEventListener("click", closeModal);
    }

    if (cancelSaveListBtn) {
        cancelSaveListBtn.addEventListener("click", closeModal);
    }
    ...
    function attachEnrichEvents() {
        ...
        function buildSendEmailForm(person, csrfToken) {
            if (!person.email) {
                return `
                    <button type="button" class="send-email-action-btn" disabled>
            ...
            return `
                <form method="POST" action="${SELECT_PERSON_FOR_EMAIL_URL}" class="d-inline">
                    ...
                    <button type="submit" class="send-email-action-btn">
                        <i class="far fa-envelope"></i> Send Email
                    </button>
                </form>
            `;
    ...
    // ── Single row Save to List button logic ──
